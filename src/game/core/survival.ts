...
export function dailySurvivalSettlement(
  ...
  hooks?: {
    ...
    /** v1.215（P1-9）：入睡前心情——抑郁判定应基于睡前状态，而非睡眠恢复后的心情 */
    preMood?: number;
  },
): DailySurvivalResult {
  ...
  const log = hooks?.log ?? (() => {});
  // v1.215：抑郁判定的心情口径 = 入睡前心情（缺省回退当前心情）
  const moodForDepression = hooks?.preMood ?? p.attrs.mood;

  // ---- 1) 抑郁：心情连续 < 30 达 7 天 ----
  if (moodForDepression < MOOD_DEPRESSION_THRESHOLD) {
