...
/** 特权档位：由高到低（供 npcPerkOf 向下回退） */
const PERK_TIERS: RelationState[] = ["close_friend", "friend", "acquaintance", "stranger"];
...
export function npcPerkOf(state: GameState, npcId: string): string | undefined {
  ...
  if (key === "blocked") return undefined;
  // v1.215 修复（P1-3）：特权按档位「向下」回退（挚友→朋友→相识→陌生人），取第一个存在的 perk。
  // 原实现向上回退到 close_friend——陌生人偶遇一次就白拿挚友级"免证书内推"，挚友反而丢低档特权。
  for (const t of PERK_TIERS.slice(PERK_TIERS.indexOf(key))) {
    const perk = def.perks[t];
    if (perk) return perk;
  }
  return undefined;
}
...
export function canBorrow(state: GameState, npcId: string): boolean {
  ...
  // v1.215 修复（P1-4）：用「当前好感推导的层级」判定——原 rel.state 在淡漠后残留 friend，
  // 导致淡漠中的"朋友"仍能借钱、够格的同事（affinity 达标但 state 未同步）反而借不了。
  if (currentState(rel) !== "friend") return false;
...
export function tickRelationshipDecay(state: GameState): string[] {
  ...
  for (const rel of state.relationships) {
    ...
    if (today - rel.lastInteractionDay > 7) {
      ...
      rel.state = currentState(rel); // v1.215：淡漠后同步状态字段，防旧值残留（P1-4）
