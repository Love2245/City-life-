...
import { recordMoney } from "./weekly"; // v1.215：赌鬼开销计入周记（P1-10）；与 weekly 的 arcGoalGap 构成环，但均在函数内按需访问，ESM 下可行
...
/** v1.215（P1-6）：收集阶段卡当前「办不了」的具体原因，用于软死局提示 */
function describeStageBlocker(state: GameState, card: StoryCardDef): string {
  const own = storyReqMet(state, card.requires);
  if (!own.ok) return own.reason ?? "前置条件未满足";
  for (const sol of card.solutions) {
    if (sol.kind === "ignore") continue;
    const r = solutionFeasible(state, card, sol);
    if (!r.ok && r.reason) return `${sol.label}：${r.reason}`;
  }
  return "推进条件尚未凑齐";
}
...
export function drawStoryCards(state: GameState): void {
  ...
  if (stageCard && cardDrawable(state, stageCard)) {
    queueCard(state, stageCard);
  } else if (
    stageCard &&
    today >= stageCard.window[0] &&
    today <= stageCard.window[1] &&
    !st.doneCards.includes(stageCard.id) &&
    !st.activeCards.some((a) => a.cardId === stageCard.id && a.status === "active")
  ) {
    // v1.215 修复（P1-6）：阶段卡因条件不足无法推进时给出方向提示，不再静默跳过——
    // 原实现 0 可行解时保底抽卡被静默忽略，玩家无提示地卡死在阶段直到硬期限主线失败。
    const blocker = describeStageBlocker(state, stageCard);
    pushLog(state, "🧩", `【${stageCard.title}】暂时还办不了——${blocker}。条件凑齐后自动推进，先安心攒钱/提升自己。`);
  }
...
export function tickStory(state: GameState): void {
  ...
  if (gAct) {
    ...
    recordMoney(state, -drain); // v1.215 修复（P1-10）：赌鬼开销计入周记支出
