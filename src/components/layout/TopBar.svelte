<script lang="ts">
  ...
  import { currentArc } from "../../game/core/story";
  ...
  /** v1.0 剧情模式：主线与良心指示 */
  const storyArc = $derived(gameState.mode === "story" ? currentArc(gameState) : undefined);
...
<header class="topbar hud-scope">
  ...
  <div class="money-box">
    ...
    {#if storyArc}
      <span class="chip story-chip">🎬 {storyArc.title}</span>
      <span class="chip conscience-chip" class:low={storyConscience < 0} class:high={storyConscience >= 4}>
        ⚖️ 良心 {storyConscience > 0 ? "+" : ""}{storyConscience}
      </span>
    {/if}
  </div>
  ...
  <div class="menu-box">
    ...
    <!-- v1.4 P2 猫咪面板入口 -->
    <button class="btn btn-ghost phone-btn cat-btn" class:has-cat={gameState.pets.length > 0} onclick={() => (uiState.modal = "cat")}>🐱</button>
...
<style>
  ...
  .story-chip {
    background: rgba(216, 132, 255, 0.14);
    color: #d9b3ff;
    font-weight: 800;
  }
  .conscience-chip {
    background: rgba(147, 160, 191, 0.14);
    color: #a8b2d6;
  }
  .conscience-chip.low {
    background: rgba(255, 107, 107, 0.15);
