<script lang="ts">
  ...
  function pick(idx: number): void {
    if (!payload) return;
    if (!card) return;
    const check = solutionFeasible(gameState, card, card.solutions[idx]);
    if (!check.ok) { showToast(check.reason ?? "条件未满足"); return; }
...
{#if payload}
  <div class="overlay hud-scope" onclick={() => (resolved ? close() : null)} transition:fade={{ duration: 200 }}>
    <div class="modal story-modal" onclick={(e) => e.stopPropagation()}>
      ...
      {:else if !resolved}
        <div class="e-head">
          <span class="gi-emoji" style="font-size:34px;line-height:1">{card.icon}</span>
