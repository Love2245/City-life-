<script lang="ts">
  ...
  // v1.3b2：数据层已统一为 30 秒基准（并由 getMiniGameConfigFor 做技能减时），此处不再叠加 x1.15
  const timeLimit = cfg.timeLimit ?? null;
  ...
  /** v1.25 键盘：数字键 1-N 选择答案 */
  function onKey(e: KeyboardEvent): void {
    const n = Number(e.key);
    if (Number.isInteger(n) && n >= 1 && n <= (cur?.options.length ?? 0)) {
      answer(n - 1);
    }
  }
...
<style>
  ...
  .quiz-opt.ok-fb {
    ...
    animation: fb-ok 0.25s ease;
  }
  .quiz-opt.bad-fb {
    border-color: #ff8a7a;
    background: rgba(255, 122, 122, 0.15);
    animation: shake-x 0.25s ease;
  }
  @keyframes fb-ok {
    0% { transform: scale(1); }
