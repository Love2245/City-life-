<script lang="ts">
  import { showToast, playSceneTransition } from "../../stores/uiStore.svelte";
  import { playSound } from "../../lib/audio";
  import { gameState } from "../../stores/gameStore.svelte";
  import { REGIONS, enterRegion } from "../../game/core/regions";
  ...
  function select(id: string): void {
    ...
    playSound("walk");
    const r = enterRegion(gameState, id);
    if (!r.ok) {
      showToast(r.reason ?? "无法前往");
      return;
    }
    // 只有真实进入一级区域时播放完整城市过场。
    playSceneTransition("bus", d.name, 1800, id);
  }
</script>
...
<div class="main-view hud-scope">
  ...
  <div class="districts">
    {#each districts as d (d.id)}
      ...
      >
        <span class="gi-emoji" style="font-size:64px;line-height:1">{d.icon}</span>
