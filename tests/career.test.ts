...
describe("发薪", () => {
  ...
  it("v1.066：非发薪日 paycheck 不发薪（周结不再每晚发薪）", () => {
    const { s } = hiredWeekly();
    const job = getJobDef("job_factory_qc")!;
    s.time.day = 2;
    applyJob(s, job); // 上班 1 天
    const moneyBefore = s.player.money;
    s.time.day = 3; // 非发薪日
    const r = paycheck(s);
    expect(r.ok).toBe(false);
    expect(s.player.money).toBe(moneyBefore); // 分文未发
    expect(s.career.workedDays).toBe(2); // 出勤保留（入职 1 天 + 上班 1 天），等待发薪日
  });
});
