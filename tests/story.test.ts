...
describe("v1.0 剧情模式：寄钱与目标", () => {
  ...
  it("v1.066：justice_s2 收下钱解法也设 evidence_bribe 证据（不再死局）", () => {
    const s = storyState("seek_justice");
    s.story.stage = 2; // justice_s2 的 stage=2
    s.flags["story_evidence_witness"] = true;
    s.flags["story_evidence_ledger"] = true;
    s.story.activeCards = [{ cardId: "justice_s2", triggeredDay: 0, deadlineDay: 99, status: "active" }];
    // 选第 3 个解法：收下钱，就此罢手
    const r = resolveStoryCard(s, "justice_s2", 2);
    expect(r.ok).toBe(true);
    expect(s.flags["story_evidence_bribe"]).toBe(true);
    // 三证据齐全 → 目标可达
    expect(arcGoalMet(s)).toBe(true);
  });
});
