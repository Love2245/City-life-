...
describe("日历系统 v0.95（纪元 2026-08-01）", () => {
  ...
  it("v1.215 修复：跨月后 1 号天气按新月份季节重掷（8月→9月不出现夏季专属热浪）", () => {
    const s = createInitialState(1);
    s.time.month = 8;
    s.time.day = 30;
    s.time.hour = 23;
    advanceHours(s, 2); // 23:00 → 次日 01:00，跨到 9 月 1 号
    expect(s.time.month).toBe(9);
    expect(s.time.day).toBe(1);
    // 原实现先掷天气再进位月份 → 1 号错用上月（夏）季节；修复后按 9 月（秋）池重掷，不会出现 heatwave
    expect(s.weather.id).not.toBe("heatwave");
  });
