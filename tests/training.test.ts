...
import { checkRequirements } from "../src/game/core/actions";
...
describe("v1.10 培训学校", () => {
  ...
  it("v1.20 培训证书为万能证书：解锁所有需要技能证书的岗位（含技能门槛）", () => {
    const s = student();
    s.flags["item_training_cert"] = true;
    s.player.skills.service = 0; // 不满足咖啡厅 service 3
    s.player.attrs.health = 80;
    s.player.attrs.stamina = 80;
    s.time.day = 3; // 周一（咖啡厅固定岗周六休，避开休息日）
    // 手机招聘校验路径
    const job = getJobDef("job_cafe_staff")!;
    const ck = checkRequirements(s, job.requirements, { jobId: job.id });
