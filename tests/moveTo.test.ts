...
describe("moveTo 地点入口", () => {
  ...
  it("游乐场已开放可进入（P1-14：修复误锁导致内容不可达）", () => {
    const s = createInitialState();
    s.time.hour = 12;
    expect(moveTo(s, "suburb").ok).toBe(true);
    expect(moveTo(s, "suburban_edge").ok).toBe(true);
    const r = moveTo(s, "amusement_park");
    expect(r.ok).toBe(true);
    expect(s.locationId).toBe("amusement_park");
  });
...
describe("v1.4 P4 外出移动触发野生对决", () => {
  it("携带猫外出移动时可能触发对决并置标记", () => {
    const s = createInitialState(7);
    s.time.hour = 12;
    adoptCat(s, "orange");
    setActivePet(s, s.pets[0].uid);
    // 携带猫咪可进入允许宠物的地点（公园），禁猫场所（市场等）会被拦截
    const r = moveTo(s, "park");
    expect(r.ok).toBe(true);
    // 概率触发：若触发则 catBattle 已建立且标记置位
    if (s.catBattle) {
      expect(s.flags["cat_battle_pending"]).toBe(true);
      expect(s.catBattle.finished).toBe(false);
    }
  });
