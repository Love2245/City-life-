...
import { rentVehicle, checkVehicleExpiry } from "../src/game/core/vehicle";
...
describe("v0.936 · 租车跨月（月份进位）", () => {
  it("月底租用仍可用满 30 天，不提前到期", () => {
    const s = createInitialState();
    s.player.money = 1000;
    ...
    s.time.day = 29;
    checkVehicleExpiry(s);
    expect(s.flags["item_e_bike"]).toBe(false); // 次月 29 日才到期
  });
});
