...
<div class="overlay hud-scope" onclick={() => (uiState.modal = null)}>
  <div class="modal" onclick={(e) => e.stopPropagation()}>
    ...
    <div class="slider-box">
      <div class="slider-head">
        ...
        <span class="dim">
          {#if alarmOn}
            闹钟叫醒约 {actualWake}:00 · 体力恢复约 {recoveryPct}%
          {:else}
            醒来约 {wakeHour}:00 · 体力恢复约 {recoveryPct}%
          {/if}
        </span>
      </div>
    ...
    <div class="opts">
      ...
      <button class="opt" onclick={() => doNap()} disabled={!canNap}>
        ...
        <div class="o-info">
          ...
          <div class="o-desc dim">白天补觉（10-18 点可用），恢复上限 20%</div>
        </div>
      </button>
    </div>
  </div>
