...
export function callFriend(state: GameState): PhoneResult {
  if (state.relationships.length === 0) {
    ...
    return { ok: true, text: "翻遍通讯录，好像没什么能打电话的朋友……", deltas: collectDeltas({ mood: -2 }) };
  }
  applyEffects(state, { mood: 5, stress: -4, log: "和朋友煲了会电话粥，心情好多了" }, "📞");
  advanceTime(state, 0.5);
  return { ok: true, text: "和朋友聊了会近况，心情好多了。", deltas: collectDeltas({ mood: 5, stress: -4 }) };
}
