...
describe("v0.97 星期制度", () => {
  ...
  it("普通工作周六休息、周日可上班；双休岗周六日都休；自由接单岗不休", () => {
    const s = createInitialState();
    s.time = { ...s.time, year: 2026, month: 8, day: 1 }; // 周六
    expect(isRestDay(s, getJobDef("job_factory_line")!)).toBe(true);
