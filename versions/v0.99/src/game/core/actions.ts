...
export function performAction(
  ...
  if (action.handler === "buy_item" && action.itemId) {
    ...
    // v0.99 购入的物品同时进入「可赠送背包」（ownedItems），使送礼功能对购买物品生效
    if (!state.ownedItems.includes(action.itemId)) {
      state.ownedItems = [...state.ownedItems, action.itemId];
    }
...
function enterLocation(state: GameState, loc: LocationDef): CheckResult {
  ...
  // v0.99 偶遇：进入地点时按 NPC 的 meetLocation 偶遇（每日每 NPC 至多一次）
  maybeMeetNpc(state, loc.id, loc.name);
  return { ok: true };
}
