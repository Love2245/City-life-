...
import { statusSleepPenalty, reconcileStatuses, cureStatuses, upsertStatus, hasStatus } from "./statuses";
...
import { tickRelationshipDecay, tickRomance, tickColleagueAffinity, tickLoans, getNpcDef } from "./relationships";
...
export function sleepSettlement(
  ...
  const lodging = getEffectiveLodging(state);
  const maxStamina = getMaxStamina(state);
  ...
  // v0.99 蹭住：当日预约去好友家蹭住 → 免当晚住宿费并清零预约
  ...
  if (state.living.mode === "nightly" && !hospitalized) {
    if (crashedAt) {
      const npcName = getNpcDef(crashedAt)?.name ?? "朋友";
      const base = nightlyPrice(state);
      const coldwave = currentWeather(state) === "coldwave";
      const wouldPay = coldwave ? Math.round(base * 1.3) : base;
      pushLog(state, "🛏️", `在${npcName}家蹭住了一晚，省下 ${wouldPay} 元住宿费`);
  ...
  // 8.6.4.1) v0.99 关系衰减：超过 7 天未互动每天 -1，超过 14 天每天 -2（父母免衰减）
  for (const l of tickRelationshipDecay(state)) pushLog(state, "💔", l);

  // 8.6.4.2) v0.99 恋爱维护：已表白恋人连续 14 天不互动 → 渐行渐远（好感归 30）
  for (const l of tickRomance(state)) pushLog(state, "💔", l);

  // 8.6.4.3) v0.99 同事日久生情：在职时每天给同事类联系人 +1 好感
  for (const l of tickColleagueAffinity(state)) pushLog(state, "💞", l);

  // 8.6.4.4) v0.99 借款逾期：欠款超 30 天未还 → 好感 -30 并拉黑
