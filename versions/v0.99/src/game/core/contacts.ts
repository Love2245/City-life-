...
import { addAffinity, ensureRelationship, getNpcDef } from "./relationships";
...
export function addContact(state: GameState, id: string): boolean {
  ...
  // 赠物
  if (d.onAdd?.items) {
    for (const it of d.onAdd.items) state.ownedItems = [...state.ownedItems, it];
  }
  // 一次性效果
  ...
  // v0.99 关系系统：新增联系人同步建立 Relationship（父母免衰减、初始好感 60）
  ensureRelationship(state, id, {
    noDecay: c.relation === "parent",
    initialAffinity: c.relation === "parent" ? 60 : 0,
  });
  return true;
}
...
export function callContact(state: GameState, id: string): PhoneResult {
  ...
  // v0.99 关系系统：同步建立/取回 Relationship；父母免衰减初始好感 60
  const rel = ensureRelationship(state, id, {
    noDecay: c.relation === "parent",
    initialAffinity: c.relation === "parent" ? 60 : 0,
  });
  const today = gameDay(state.time);
  // 每 NPC 每日限 1 次有效好感（其余时间只寒暄）
  let affinityLog = "";
  if (rel.flags?.lastCallDay !== today) {
    rel.flags = rel.flags ?? {};
    rel.flags.lastCallDay = today;
    affinityLog = addAffinity(state, id, 2, { name: c.name });
  }
  onContactMessaged(state, c.id); // v0.985 恋人：主动通讯的不同对象去重累计
  applyEffects(state, { mood: 2 }, "📞");
  // 台词按关系等级从 npcs.json 取，无 NPC 定义则回退固定台词
  const npc = getNpcDef(id);
  const text = npc?.dialogues?.[rel.state] ?? line[c.relation] ?? `和${c.name}聊了几句。`;
  pushLog(state, "📞", `给${c.name}打了个电话` + (affinityLog ? ` · ${affinityLog}` : ""));
  const deltas = collectDeltas({ mood: 2 });
  if (affinityLog) deltas.push({ key: "affinity", label: `${c.name}好感`, value: 2 });
  return { ok: true, text, deltas };
}
...
export function smsContact(state: GameState, id: string): PhoneResult {
  ...
  const today = gameDay(state.time);
  const smsCount = rel.flags?.lastSmsDay === today ? ((rel.flags.lastSmsCount as number) ?? 0) : 0;
  // 每 NPC 每日限 2 次有效好感
  let affinityLog = "";
  if (smsCount < 2) {
    rel.flags = rel.flags ?? {};
    rel.flags.lastSmsDay = today;
    rel.flags.lastSmsCount = smsCount + 1;
    affinityLog = addAffinity(state, id, 1, { name: c.name });
  }
  ...
  pushLog(state, "💬", `给${c.name}发了条短信` + (affinityLog ? ` · ${affinityLog}` : ""));
  const deltas = collectDeltas({ mood: 1 });
  if (affinityLog) deltas.push({ key: "affinity", label: `${c.name}好感`, value: 1 });
  return { ok: true, text: `消息发出去了，${c.name}很快就回了你。`, deltas };
}
