...
export function canCook(state: GameState, recipeId: string): { ok: boolean; reason?: string } {
  ...
  if (!hasFacility(state, "cook")) {
    return { ok: false, reason: "当前住处没有厨房，做不了饭（需出租屋或出租公寓）" };
  }
