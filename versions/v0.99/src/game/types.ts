...
export interface Relationship {
  ...
  /** 好感度 0-100 */
  affinity: number;
  /** 关系等级（由 affinity 派生，落盘冗余便于 UI） */
  state: RelationState;
  /** 绝对日，最近一次互动（用于衰减/冷却） */
  lastInteractDay: number;
  /** 当日已获好感（防刷，单 NPC 每日上限） */
  todayGain: number;
  /** todayGain 所属绝对日（跨天清零） */
  todayGainDay: number;
  /** 单 NPC 标记：confessed 已表白 / noDecay 父母免衰减 / brokenUp 已分手 / lastCallDay / lastSmsDay / lastSmsCount */
  flags?: Record<string, boolean | number>;
}

/** NPC 定义（data/npcs.json）：与 Contact 分层并存 —— Contact=能否联系，NpcDef=邂逅/关系深度来源 */
export interface NpcDef {
  ...
  meetChance: number;
  /** 邂逅条件（可选） */
  meetConditions?: { minCharm?: number; hourRange?: [number, number] };
  /** 初始好感（v0.99 关系系统启用时写入） */
  ...
  /** 好感达到该值才交换联系方式（加入 Contact） */
  contactAt: number;
  /** 按关系等级的台词（stranger/acquaintance/friend/close_friend/lover） */
  dialogues?: Partial<Record<RelationState, string>>;
  /** 关系等级收益（阶段2/3 启用）：如 friend=borrow 可借钱，close_friend=referral_<jobId> */
  perks?: Record<string, string>;
}

/** v0.99 恋爱子系统状态 */
export interface RomanceState {
  /** 当前交往/同居的恋人 npcId（无则 undefined） */
  partnerId?: string;
  /** 各 NPC 累计约会次数 */
  dateCount: Record<string, number>;
  /** 是否同居（房租对半、每日 +3 心情 -2 压力） */
  cohabiting: boolean;
  /** v0.99 蹭住：今晚打算去该 NPC 家过夜（sleepSettlement 兑现，省住宿费） */
  crashTonight?: string;
  /** v0.99 当前借款来源 NPC（同时段只持有一笔借款） */
  loanNpc?: string;
  /** v0.99 借款绝对日（用于逾期判定） */
  loanDay?: number;
}
...
export interface Effects {
  ...
  /** v0.99 关系系统：数据驱动给某 NPC 加好感（amount 可正可负，受每日上限与边际递减约束） */
  affinity?: { npcId: string; amount: number; reason?: string };
}
...
export interface EventDef {
  ...
  /**
   * v0.99 仅剧情强制触发：为 true 时永不进入日常随机候选池，
   * 只能由 queueEvent（pendingEvents 队列）弹出。
   * 用于「弥撒散场后的教友聚餐」这类必须由特定行动引出的事件，
   * 避免玩家在街头闲逛时凭空抽到教堂剧情。
   */
  forcedOnly?: boolean;
}
