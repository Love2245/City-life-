...
import { addAffinity } from "./core/relationships";
...
export function applyEffects(state: GameState, effects: Effects, icon = "📝"): void {
  ...
  // v0.99 关系系统：数据驱动给某 NPC 加好感（relationships 不 import engine，避免成环）
  if (effects.affinity) {
    const log = addAffinity(state, effects.affinity.npcId, effects.affinity.amount, { reason: effects.affinity.reason });
    if (log) pushLog(state, "💞", log);
  }
  if (effects.log) {
