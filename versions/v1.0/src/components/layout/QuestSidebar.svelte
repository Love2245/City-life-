            ...
            {#if storyArc}
              <div class="story-section">
                <div class="story-head">
                  ...
                  {#if storyArc.goalTotal > 0}
                    <span class="story-goal dim">目标资金 {formatMoney(gameState.story.goalFund)}/{formatMoney(storyArc.goalTotal)}</span>
                  {:else}
                    <span class="story-goal dim">目标：{storyArc.goal}</span>
                  {/if}
                </div>
                {#if storyArc.goalTotal > 0}
                  <div class="story-bar"><div class="story-fill" style="width:{Math.min(100, (gameState.story.goalFund / storyArc.goalTotal) * 100)}%"></div></div>
                {/if}
                <div class="story-meta dim">
                  <span>阶段 {gameState.story.stage + 1}/5 · {storyArc.stages[gameState.story.stage]}</span>
