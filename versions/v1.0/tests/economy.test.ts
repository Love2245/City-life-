...
describe("经济闭环 v0.94（产权房 / 购车 / 投资 / 奢侈品）", () => {
  ...
  describe("购车资产", () => {
    ...
    it("租用车到期正常回收（回归）", () => {
      ...
      checkVehicleExpiry(s);
      expect(s.vehicles["car"]).toBeUndefined();
      expect(s.flags["item_car"]).toBe(false);
    });
  ...
  describe("投资理财", () => {
    ...
    it("股票波动可能亏损但市值保底", () => {
      ...
      // 波动范围：期望 2% ± 24%（两次 rng 平均 → 最大 ±24%）
      expect(Math.abs(after - before)).toBeLessThanOrEqual(Math.round(before * 0.24) + 1);
    });
