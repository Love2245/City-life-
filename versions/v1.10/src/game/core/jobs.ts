...
export function checkJobRequirements(
  ...
  // 证书门槛：内推（referral_<jobId>）可免证书直接入职
  if (req?.flag && !intern && !state.flags[req.flag] && !isReferred(state, job.id)) {
...
/** 岗位支持的载具档（incomeTiers 键）；当前车辆不在其中时回退到岗位支持的最高档 */
export function resolveTierForJob(state: GameState, job: JobDef): VehicleType {
  ...
  // 回退：从高到低找岗位支持的第一档（持 car 跑外卖 → e_bike）
