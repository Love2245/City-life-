...
describe("v0.981 宗教系统", () => {
  ...
  it("弥撒散场直接结算聚餐效果（v1.05 合并，不再进事件队列）", () => {
    const s = fresh();
    const satietyBefore = s.player.attrs.satiety;
    afterChurchMass(s);
    // v1.05：聚餐效果直接 applyEffects，不再推入 pendingEvents
    expect(s.player.attrs.satiety).toBeGreaterThan(satietyBefore);
    expect(s.flags.church_attended).toBe(1);
  });
