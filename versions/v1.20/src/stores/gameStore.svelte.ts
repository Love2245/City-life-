...
/** 当前视图路由 */
export type View = "menu" | "game" | "ending" | "saveLoad" | "settings" | "background" | "fate" | "memoir" | "storyIntro" | "weeklyReport";
...
export function confirmStoryStart(): void {
  ...
  pushLog(gameState, "📜", `剧情开始：${arc?.title ?? "主线"}。先找份活干，攒钱/学技能推进目标；每 5~9 天会有「人情债」找上门，打开手机【人情债】处理`);
  pushLog(gameState, "💡", "在手机任务栏可随时查看：阶段进度、目标进度、下次来事倒计时");
  currentView.name = "game";
}
