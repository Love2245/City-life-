...
const GOAL_POOL: Omit<DailyGoal, 'done'>[] = [
  ...
  { id: 'talk_someone', text: '跟人聊聊天', check: s => s.relationships.some(r => r.lastInteractionDay !== undefined && r.lastInteractionDay >= (typeof s.flags['goal_day']==='number'?s.flags['goal_day'] as number:0)), reward: { mood: 2 } },
];
...
export function generateDailyGoals(state: import('../types').GameState): DailyGoal[] {
  ...
  if ((state.flags['goal_day'] as number ?? -1) === today && Array.isArray((state as any).dailyGoals) && (state as any).dailyGoals.length > 0) {
    return (state as any).dailyGoals;
  }
...
export function tickDailyGoals(state: import('../types').GameState, applyReward: (mood: number) => void): number {
  const goals: DailyGoal[] = (state as any).dailyGoals ?? [];
  let completed = 0;
