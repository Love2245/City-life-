...
function monthlyWorkableDays(state: GameState, restDays: string[]): number {
  const t = state.time;
  let workable = 0;
  for (let d = 1; d <= 30; d++) {
    const w = weekdayOf({ year: t.year, month: t.month, day: d });
...
export function paycheck(state: GameState): { ok: boolean; amount: number; bonus: boolean } {
  ...
  if (!isPayday(state)) return { ok: false, amount: 0, bonus: false };
  ...
  const workable = c.kind === "weekly" ? 7 - restDays.length : monthlyWorkableDays(state, restDays);
